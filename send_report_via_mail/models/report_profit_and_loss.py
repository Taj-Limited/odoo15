...
class ReportTajProfitExcel(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet.write(0, 3, 'Year', bold)
        sheet.write(0, 4, 'Operating Income', bold)
        sheet.write(0, 5, 'Total Going', bold)
        sheet.write(0, 6, 'Total Return', bold)
        sheet.write(0, 7, 'Total Fuel', bold)
        sheet.write(0, 8, 'Total Cost', bold)
        sheet.write(0, 9, 'Cross Profit', bold)
        sheet.write(0, 10, '%', bold)
        ...
        for obj in data['products']:
            ...
            sheet.write(row, 3, obj['date'])
            sheet.write(row, 4, obj['operating_income'])
            sheet.write(row, 5, obj['total_going'])
            sheet.write(row, 6, obj['total_return'])
            sheet.write(row, 7, obj['total_fuel'])
            sheet.write(row, 8, obj['total_cost'])
            sheet.write(row, 9, obj['cross_profit'])
            sheet.write(row, 10, obj['cross_profit'] / obj['operating_income'] if obj['operating_income'] != 0 else 0)
